...
class Sequential:
    ...
    def genDeltaOptimizer(self, learning_rate, momentum_factor):
        self.computeDelta()
        for l in range(1, self.L+1):
    ...
    def adamOptimizer(self, learning_rate, rho1, rho2, epsilon):
        self.pm1 *= rho1
        self.pm2 *= rho2
        self.computeDelta()
        for l in range(1, self.L+1):
    ...
    def fit_r(self, in_v, out_v, E):
        for epoch in range(E):
            ...
            for x, y in zip(in_v, out_v):
                ...
                error += abs(self.t - self.s[self.L][0])
                if epoch == E-1 : 
    ...
    def fit(self, in_v, out_v, valin, valo, E):
        ...
        for epoch in range(E):
            ...
            for x, y in zip(in_v, out_v):
                ...
                # dropout = [0]
                # for l in range(1, self.L+1):
                #     siz = self.nL[l]*self.nL[l-1]
                #     shuff = (np.arange(siz, dtype=int))
                #     np.random.shuffle(shuff)
                #     dropout.append(np.zeros(siz, dtype=bool))
                #     dropout[l][shuff[:int(0.2*siz)]] = True
                #     dropout[l] = np.reshape(dropout[l], np.shape(self.W[l]))
                #     self.W[l][dropout[l]] = 0
                ...
                self.s[0] = np.copy(np.reshape(x, (np.shape(x)[0], 1)))
                self.fwprop_fn()
                self.bkprop_fn(self, 0.001, 0.9, 0.999, 1e-8)
                # self.bkprop_fn(self, 0.01, 0.01)
                
                # # dropout recovery:
                # for l in range(1, self.L+1):
                #     self.W[l][dropout[l]] = curr_w[l][dropout[l]]
